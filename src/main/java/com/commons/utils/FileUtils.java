...
public class FileUtils {
	...
	public static final String DOWN_URL_API="/api/file/file_download?filePath=";
	...
	public static String upload(MultipartFile file){
		int size=StringUtils.isNotBlank(env.getProperty("file.upload-size"))?Integer.valueOf(env.getProperty("file.upload-size")):15;
		if(!Tools.checkFileSize(file.getSize(),size,"M")){
			return "文件不能大于"+size+"M";
		}
		String folder=File.separator+DateUtils.getDate()+File.separator;
		//文件名
		String fileName = IdGen.uuid()+Tools.getFileSuffix(file.getOriginalFilename());  //文件名字
		//CommonsMultipartFile cf= (CommonsMultipartFile)file;
		//DiskFileItem fi = (DiskFileItem)cf.getFileItem();
		//File cfile = fi.getStoreLocation();
		
		String path = env.getProperty("file.upload-path")+ folder;
		path=path(path);
		//检查目录是否存在，不存在则创建一个
	 ...
	 * @param request 请求对象
	 * @param response 响应对象
	 * @param fileName 指定下载的文件名0..
	 * @return 返回错误信息，无错误信息返回null
	 */
	public static String downFile(String dataPath, HttpServletRequest request, HttpServletResponse response, String fileName) throws Exception{
		File file=new File(path(env.getProperty("file.upload-path")+dataPath));
		return fileDown(request, response, fileName, file);
	}

	public static String downTempFile(String dataPath, HttpServletRequest request, HttpServletResponse response, String fileName) throws Exception{
		String path=path(ResourceUtils.getURL("classpath:").getPath());
		File file=new File(path+dataPath);
		return fileDown(request, response, fileName, file);
	}
	
	private static String fileDown(HttpServletRequest request, HttpServletResponse response, String fileName,
		...
		try {
			...
			try {
				 ...
				 * 所以，我们忽略这种异常
				 */
				 throw new Exception("提醒：向客户端传输时出现IO异常，但此异常是允许的，有可能客户端取消了下载，导致此异常，不用关心！");
			}
		} catch (Exception e) {
